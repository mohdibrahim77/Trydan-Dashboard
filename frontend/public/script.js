...
function initializeMap() {
    ...
    setTimeout(() => {
        ...
        racingLinePolyline = L.polyline(NMIT_TRACK, {
            color: '#00ff88',
            weight: 6,
            opacity: 0.9,
            dashArray: '10, 5',
        ...
        // Add START marker
        L.marker([13.129051169254026, 77.58824465835409], {
            icon: L.divIcon({
        ...
        kartMarker = L.marker([13.129051169254026, 77.58824465835409], {
            icon: L.divIcon({
...
function addCornerMarkers() {
    const corners = [
        { lat: 13.127103, lon: 77.58793, name: '🔄 Turn 1', color: '#FFD700' },
        { lat: 13.127111, lon: 77.58755, name: '🔄 Turn 2', color: '#FFD700' },
        { lat: 13.127823, lon: 77.58600, name: '🔄 Hairpin', color: '#ff3366' },
        { lat: 13.129186, lon: 77.58633, name: '🔄 Turn 4', color: '#FFD700' }
    ];
    ...
    corners.forEach(corner => {
        L.circleMarker([corner.lat, corner.lon], {
            ...
            opacity: 0.9,
            fillOpacity: 0.6
        }).addTo(map).bindPopup(corner.name);
    });
}
